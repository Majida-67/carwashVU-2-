...
<head>
    ...
    <style>
        ...
        .status-rescheduled { color: #007bff; }
        .btn-group {
            display: flex;
        ...
        .reschedule-btn { background-color: #ffc107; color: black; }
        .delete-btn { background-color: #6c757d; }
        .history-btn { background-color: #007bff; }
    </style>
    <script>
        function deleteBooking(id) {
            if (confirm('Are you sure you want to delete this booking?')) {
                fetch('delete_booking1.php?id=' + id, { method: 'GET' })
                        ...
                        <td>
                            <span class="status 
                                <?php echo ($row['status'] == 'Pending') ? 'status-pending' :
                                            ...
                                            ($row['status'] == 'Cancelled' ? 'status-cancelled' : 'status-rescheduled')); ?>">
                                <?php echo $row['status']; ?>
                            </span>
                        </td>
                        <td>
